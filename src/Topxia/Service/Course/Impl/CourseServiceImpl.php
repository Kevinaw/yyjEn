...
class CourseServiceImpl extends BaseService implements CourseService
{
	...
	public function findUserLeaningCourses($userId, $start, $limit)
	{
		...
		// 课程试听者
// 		$members1 = $this->getMemberDao()->findMembersByUserIdAndRoleAndIsLearned($userId, 'listener', '0', $start, $limit);
// 		if(!empty($members1))
// 		{
...
// 		}
	...
	public function findLockedListeners()
	{
		$LockedListeners = $this->getMemberDao()->findMembersByRoleAndLocked('listener', 1);
		
		return $LockedListeners;
	}
	...
	public function auditCourse($id)
	{
		$course = $this->tryManageCourse($id);
		$this->getCourseDao()->updateCourse($id, array('status' => 'audit'));
		$this->getLogService()->info('course', 'apply', "申请课程《{$course['title']}》(#{$course['id']})");
	}
	...
	public function deleteCourseMember($id)
	{
		return $this->getMemberDao()->deleteMember($id);
	}
	...
	public function findCourseStudentsAndListeners($courseId, $start, $limit)
	{
		return array_merge($this->getMemberDao()->findMembersByCourseIdAndRole($courseId, 'student', $start, $limit), $this->getMemberDao()->findMembersByCourseIdAndRole($courseId, 'listener', $start, $limit));
	}
	...
	// 查找代课老师
	public function findCourseAltTeacher($courseId)
	{
		return $this->getMemberDao()->findMembersByCourseIdAndRole($courseId, 'altteacher', 0, self::MAX_TEACHER);
	}
	...
	public function addCourseListener($courseId, $userId)
	{
		// locked = 1 and role = listener 说明是试听申请者。
		$member = array(
				'courseId' => $courseId,
				'userId' => $userId,
				'orderId' => 0,
	...
	//设置教师以及代课教师
	// $data = array('absent'=>, 'altteacher'=>);
	public function setCourseTeacherAndAltteacher($courseId, $data)
	{
		// 过滤数据
		$absent = isset($data['absent']) ? $data['absent'] : 0;
		$altteacher = isset($data['altteacher']) ? $data['altteacher'] : '';
		
		$userAlt = $this->getUserService()->getUserByNickname($altteacher);
		if($absent && empty($userAlt)) {
			throw $this->createServiceException("代课教师不存在或没有教师角色，设置课程(#{$courseId})教师失败");
		}elseif (!$absent && !empty($userAlt)){
			throw $this->createServiceException("授课教师缺课？否则不能设置代课教师。设置课程(#{$courseId})教师失败");
		}
		// 授课教师和代课教师不能是同一个人
		$existTeacherMember = $this->findCourseTeachers($courseId);
		if ($existTeacherMember[0]['userId'] == $userAlt['id']){
			throw $this->createServiceException("授课教师与代课教师不能为同一人。设置课程(#{$courseId})教师失败");;
		}
		...
		// 先清除所有的已存在的代课教师会员
		$existTeacherMember = $this->findCourseAltTeacher($courseId);
		if(!empty($existTeacherMember)){
			$this->getMemberDao()->deleteMember($existTeacherMember[0]['id']);
		}

		if($absent){
			// 插入新的代课教师
			$this->getMemberDao()->addMember($member);
			$this->getLogService()->info('course', 'update_teacher', "添加课程#{$courseId}的代课教师", $member);
			// 更新课程的alterTeacherId，该字段为课程可见教师的ID列表
			$fields = array('teacherAbsent' => $absent, 'alterTeacherId' => $member['userId']);
			$this->getCourseDao()->updateCourse($courseId, CourseSerialize::serialize($fields));
		}else{
			$this->getLogService()->info('course', 'update_teacher', "清除课程#{$courseId}的代课教师", $member);
			// 更新课程的alterTeacherId，该字段为课程可见教师的ID列表
			$fields = array('teacherAbsent' => $absent, 'alterTeacherId' => NULL);
			$this->getCourseDao()->updateCourse($courseId, CourseSerialize::serialize($fields));			
		}	
	}
...
// 		}
	...
	public function tryListenCourse($courseId)
	{
		$course = $this->getCourseDao()->getCourse($courseId);
		if (empty($course)) {
			throw $this->createNotFoundException();
		...
		//课程中有这个学员，不能申请试听
		if (!empty($member)) {
			throw $this->createAccessDeniedException('您已经是课程学员，不能再申请试听本课程');
		}
...
class CourseSerialize
{
    public static function serialize(array &$course)
    {
    	...
    	if (isset($course['startingTime'])) {
    		if (is_string($course['startingTime']) and !empty($course['startingTime'])) {
    			$course['startingTime'] = strtotime($course['startingTime']);
    		} else {
    			$course['startingTime'] = time();
    		}
    	}
    ...
    public static function unserialize(array $course = null)
    {
		...
		if(0 == $course['startingTime']){
			$course['startingTime'] = date('Y-m-d H:i', time());
		} else {
			$course['startingTime'] = date('Y-m-d H:i', $course['startingTime']);
		}
