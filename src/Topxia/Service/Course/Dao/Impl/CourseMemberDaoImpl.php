...
class CourseMemberDaoImpl extends BaseDao implements CourseMemberDao
{
    ...
    public function findMembersByRoleAndLocked($role, $locked)
    {
    	$sql = "SELECT * FROM {$this->table} WHERE role = ? AND locked = ?";
    	return $this->getConnection()->fetchAll($sql, array($role, $locked));
    }
    ...
    public function findMembersByUserIdAndCourseIdAndRoleAndIsLearned($userId, $courseId, $role, $isLearned)
    {
    	$sql = "SELECT * FROM {$this->table} WHERE userId = ? AND courseId = ? AND role = ? AND isLearned = ?";
    	return $this->getConnection()->fetchAssoc($sql, array($userId, $courseId, $role, $isLearned)) ? : null;
    }
