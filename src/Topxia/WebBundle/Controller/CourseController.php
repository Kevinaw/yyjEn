...
class CourseController extends BaseController
{
    ...
    public function listenApplyAction(Request $request, $id)
    {
    	$course = $this->getCourseService()->tryListenCourse($id);
    
    	$currentUser = $this->getCurrentUser();
    	
    	$this->getCourseService()->addCourseListener($id, $currentUser['id']);

    	// 发送邮件
    	$user = $this->getUserService()->getUserByRoles('ROLE_ADMIN');
		$this->sendEmail($user['email'], '有新的课程试听申请', '有新的课程试听申请，请登录管理后台进行审核！');
    
    	return $this->render('TopxiaWebBundle:Course:listenapply-success.html.twig');
    }
