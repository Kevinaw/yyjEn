...
class CourseManageController extends BaseController
{
    ...
    public function classroomAction(Request $request, $id)
    {    	
    	$course = $this->getCourseService()->tryManageCourse($id);
    	$form = $this->createClassroomForm($course);
    	
    	if($request->getMethod() == 'POST'){

    		$form->bind($request); 
    		if($form->isValid()){
    			$classroomInfo = $form->getData();
   			 
    			$this->getCourseService()->updateCourse($id, $classroomInfo);
    			$this->setFlashMessage('success', '课程教室信息已保存！');
    			return $this->redirect($this->generateUrl('course_manage_classroom',array('id' => $id)));
    		}
    	}
    	return $this->render('TopxiaWebBundle:CourseManage:classroom.html.twig', array(
    			'course' => $course,
    			'form' => $form->createView(),
    	));
    }
    ...
    public function teachersAction(Request $request, $id)
    {
        ...
        if($request->getMethod() == 'POST'){
			...
			$this->getCourseService()->setCourseTeacherAndAltteacher($id, $data);
        ...
        if(!empty($users)){
        	$teacher['id'] = $teacherMembers[0]['userId'];
        	$teacher['nickname'] = $users[$teacher['id']]['nickname'];
        }
        
        $alterTeacherMembers = $this->getCourseService()->findCourseAltTeacher($id);
        $altTeacherUser = $this->getUserService()->findUsersByIds(ArrayToolkit::column($alterTeacherMembers, 'userId'));

        $altTeacher = array(
        	'id' => '',
        	'nickname' =>''
        );      
        if(!empty($altTeacherUser)){
        	$altTeacher['id'] = $alterTeacherMembers[0]['userId'];
        	$altTeacher['nickname'] = $altTeacherUser[$alterTeacherMembers[0]['userId']]['nickname'];
        }

        return $this->render('TopxiaWebBundle:CourseManage:teachers.html.twig', array(
            'course' => $course,
            'teacher' => $teacher,
        	'altTeacher' => $altTeacher,
        	'teacherAbsent' => ($course['teacherAbsent'] ? true : false),
        ));
    }
    ...
    {
    	$this->getCourseService()->auditCourse($id);
    	
    	//send email
    	$email = $this->getUserService()->getAdminEmail();
    	$this->sendEmail(
    			$email,
    			"有新课程发布申请",
    			"请审核申请发布的新课程"
...
// 			->add('tags', 'tags')
//             ->add('expiryDay', 'text')
            ->add('startingTime', 'text')
            ->add('length', 'choice', array(
			'choices' => array('20' => '20分钟', 
					'40' => '40分钟',
					'60' => '60分钟',
					'90' => '90分钟',
					'120' => '120分钟',),
	...
	private function createClassroomForm($course)
	{
		$builder = $this->createNamedFormBuilder('course', $course)
		->add('classroomId', 'classroom', array(
				'empty_value' => '请选择教室'
		));
